 ...
 * @param options
 */
function createDistortableImageOverlay(
	L: any,
	map: { project: (arg0: any) => any; unproject: (arg0: any) => any },
	options = {}
) {
	//参数校验
	if (Object.prototype.toString.call(options) !== '[object Object]') throw TypeError();
	//根据鼠标位置、图片大小计算图片四角的位置
	const { img, center, ratio, height } = options as any;
	const centerPoint = map.project(center);
	const imgHeight = height;
	const imgWidth = (ratio * imgHeight).toFixed(0);
...
export default function (L: any) {
	return {
		...
		_fireCreatedEvent: function () {
			createDistortableImageOverlay(L, this._map, {
				img: this._template,
		...
		//鼠标移动事件: 图片跟随鼠标移动
		_onMouseMove: function (e: { latlng: string }) {
			let latlng = e.latlng;
